...
class atlas_i2c:
    long_timeout = 2  # the timeout needed to query readings and
                        #calibrations
    short_timeout = 1  # timeout for regular commands
    default_bus = 1  # the default bus for I2C on the newer Raspberry Pis,
                     # certain older boards use bus 0
    default_address = 100 # the default address for the pH sensor #99 for PH Sensor, #100 for EC Sensor
    ...
    def read(self, num_of_bytes=31):
        ...
        if response[0] == 1:  # if the response isnt an error
            ...
            return "Current EC = " + ''.join(char_list)
        else:
